...
import {
  Star, Shield, Sparkles, Layers, Heart, ChevronRight
} from "lucide-react";
import App from "./App.jsx";
import { DEMO_CONFIG } from "./demo.config.js";

const iconMap = { Sparkles, Layers, Heart, Shield, Star };
...
export default function DemoWrapper() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f5f5", fontFamily: "'DM Sans', system-ui, sans-serif", color: "#1a1a1a" }}>
      ...
      <aside className="demo-sidebar" style={{
        ...
        <div style={{ padding: "16px 24px 0" }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: accent, background: `${accent}12`, padding: "4px 10px", borderRadius: 4 }}>
            Prototype Demo
          </span>
        </div>
        ...
        <div style={{ padding: "20px 24px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{
              ...
              fontFamily: DF, fontSize: 22, fontWeight: 700, color: "#fff"
            }}>
              {c.studio.logo}
            </div>
            <div>
              <div style={{ fontFamily: DF, fontSize: 18, fontWeight: 600, letterSpacing: "0.02em", color: "#111827" }}>{c.studio.name}</div>
      ...
      <main style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "flex-start", paddingTop: 32, paddingBottom: 32, marginLeft: 320, marginRight: 340 }}>
        <div style={{ position: "relative" }}>
          <div style={{
            ...
            <div style={{
              width: 390, height: 720, borderRadius: 40, overflow: "hidden",
      ...
      {/* RIGHT SIDEBAR */}
      <aside className="demo-sidebar" style={{
        width: 340, flexShrink: 0, background: "#fff", borderLeft: "1px solid #e5e7eb",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            return (
              <div key={i} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: "18px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <h3 style={{ fontFamily: DF, fontSize: 18, fontWeight: 600, color: "#111827", margin: 0 }}>{card.title}</h3>
                </div>
                <p style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.6, margin: 0 }}>{card.description}</p>
                {isAdminCard && (
                  <button onClick={() => setIsFullAdmin(true)} style={{
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                    width: "100%", padding: "10px 0", marginTop: 14, borderRadius: 8,
                    border: "none", background: accent, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer"
                  }}>
          ...
          <div style={{
            background: `linear-gradient(135deg, ${accent}12, ${accentDark}08)`,
            border: `1px solid ${accent}30`, borderRadius: 14, padding: "22px 18px", textAlign: "center"
          }}>
            <h3 style={{ fontFamily: DF, fontSize: 22, fontWeight: 600, color: "#111827", margin: "0 0 6px" }}>{c.cta.heading}</h3>
